...
export default function HomePage() {
  ...
  useEffect(() => { loadProducts(true); }, [selectedCategory, sortBy]);
  ...
  return (
    <main className="w-full min-h-screen bg-gray-50 px-4 md:px-10 py-8">
      <div className="flex gap-4 items-center justify-between mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-slate-900 uppercase tracking-tighter">Food Explorer</h1>
        ...
        <form onSubmit={(e) => { e.preventDefault(); loadProducts(true); }} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-bold">
            <div className="space-y-2">
              ...
              <select className="w-full p-3 border-2 border-slate-900 rounded-xl text-slate-900" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}>
            ...
            <div className="space-y-2">
              ...
              <select className="w-full p-3 border-2 border-slate-900 rounded-xl text-slate-900" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                <option value="unique_scans_n">Most Popular</option>
                <option value="product_name">Name (A-Z)</option>
                <option value="nutrition_grades">Nutrition Grade</option>
              </select>
            </div>
          </div>
        </form>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {products.map((product, idx) => <ProductCard key={`${product.code}-${idx}`} product={product} />)}
        {loading && Array(8).fill(0).map((_, i) => <Skeleton key={i} />)}
      </div>

      {!loading && products.length > 0 && (
        <div className="mt-16 text-center pb-12">
          <button onClick={() => loadProducts()} className="px-12 py-4 rounded-xl text-slate-900 border-2 border-slate-900 font-black uppercase tracking-widest hover:translate-y-1 hover:shadow-none transition-all shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] active:translate-y-1">Load More</button>
        </div>
      )}
    </main>
  );
